...
{
    hpgl_real_t x_center, y_center, sweep, x_current, y_current, chord_angle =
        5;
    ...
    radius = hpgl_compute_distance(x_current, y_current, x_center, y_center);
    ...
    hpgl_call(hpgl_add_arc_to_path(pgls, x_center, y_center,
                                   ...
                                   chord_angle,
                                   false, pgls->g.move_or_draw, true));
          ...
          hpgl_plot_function_t func, bool is_PA)
{
    ...
    while (hpgl_arg_units(pgls->memory, pargs, &x) &&
        ...
        if (current_units_out_of_range(x) || current_units_out_of_range(y))
            return e_Range;
        ...
        switch (ch & 127 /* per spec */ ) {
            case ';':
                ...
                if (pargs->phase & pe_entered)
                    hpgl_restore_pen_state(pgls, &pgls->g.pen_state,
                                           hpgl_pen_relative);
...
/* Initialization */
static int
pgvector_do_registration(pcl_parser_state_t * pcl_parser_state,
                         gs_memory_t * mem)
{
    /* Register commands */
